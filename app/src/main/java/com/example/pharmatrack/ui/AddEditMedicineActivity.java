...
public class AddEditMedicineActivity extends AppCompatActivity {
    MedicineDao dao; EditText n,m,q,p; long id = -1;
    @Override protected void onCreate(Bundle saved){ super.onCreate(saved); setContentView(R.layout.activity_add_edit_med);
        dao=new MedicineDao(this);
        n=findViewById(R.id.etName); m=findViewById(R.id.etManufacturer); q=findViewById(R.id.etQty); p=findViewById(R.id.etPrice);
        id=getIntent().getLongExtra("medId",-1);
        if(id!=-1) load();
        findViewById(R.id.btnSave).setOnClickListener(v->save());
    }
    void load(){ for(MedicineItem mi:dao.all()) if(mi.id==id){ n.setText(mi.name); m.setText(mi.manufacturer); q.setText(String.valueOf(mi.qty)); p.setText(String.valueOf(mi.price));}}
